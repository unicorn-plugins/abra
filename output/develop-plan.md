...
- [Abra 플러그인 개발 계획서](#abra-플러그인-개발-계획서)
  ...
  - [8. 스킬 상세 설계](#8-스킬-상세-설계)
    - [8.1 core (핵심스킬)](#81-core-핵심스킬)
      - [Step 1: 의도 감지 (Intent Detection)](#step-1-의도-감지-intent-detection)
      - [Step 2: 스킬 매칭 (Skill Matching)](#step-2-스킬-매칭-skill-matching)
      - [Step 3: 위임 → Skill: {매칭된 스킬}](#step-3-위임--skill-매칭된-스킬)
    - [8.2 setup (설정스킬)](#82-setup-설정스킬)
      - [Step 1: install.yaml 로드](#step-1-installyaml-로드)
      - [Step 2: Dify 접속 정보 수집](#step-2-dify-접속-정보-수집)
    ...
    - [8.3 dify-setup (설정스킬)](#83-dify-setup-설정스킬)
      ...
      - [Step 2: Dify 소스 확인](#step-2-dify-소스-확인)
      - [Step 3: 환경 변수 파일 생성](#step-3-환경-변수-파일-생성)
      - [Step 4: Docker Compose 실행](#step-4-docker-compose-실행)
      - [Step 5: 컨테이너 상태 확인 및 헬스체크](#step-5-컨테이너-상태-확인-및-헬스체크)
      - [Step 6: 초기 설정 안내](#step-6-초기-설정-안내)
      - [Step 7: 결과 보고](#step-7-결과-보고-1)
    - [8.4 scenario (지휘자스킬)](#84-scenario-지휘자스킬)
      - [Phase 0: 입력 수집](#phase-0-입력-수집)
      - [Phase 1: 시나리오 생성 → Agent: scenario-analyst](#phase-1-시나리오-생성--agent-scenario-analyst)
      - [Phase 2: 사용자 선택](#phase-2-사용자-선택)
      - [Phase 3: 저장 및 완료](#phase-3-저장-및-완료)
    - [8.5 dsl-generate (지휘자스킬)](#85-dsl-generate-지휘자스킬)
      - [Phase 0: 입력 확인](#phase-0-입력-확인)
      - [Phase 1: DSL 생성 → Agent: dsl-architect](#phase-1-dsl-생성--agent-dsl-architect)
      - [Phase 2: 검증 및 완료](#phase-2-검증-및-완료)
    - [8.6 prototype (지휘자스킬)](#86-prototype-지휘자스킬)
      - [Phase 0: 입력 확인](#phase-0-입력-확인-1)
      - [Phase 1: 프로토타이핑 실행 → Agent: prototype-runner](#phase-1-프로토타이핑-실행--agent-prototype-runner)
      - [Phase 2: 결과 확인 및 보고](#phase-2-결과-확인-및-보고)
    - [8.7 dev-plan (지휘자스킬)](#87-dev-plan-지휘자스킬)
      - [Phase 0: 입력 확인](#phase-0-입력-확인-2)
      - [Phase 1: 비기능요구사항 수집](#phase-1-비기능요구사항-수집)
      - [Phase 2: 개발계획서 작성 → Agent: plan-writer (`/oh-my-claudecode:ralplan` 활용)](#phase-2-개발계획서-작성--agent-plan-writer-oh-my-claudecoderalplan-활용)
      - [Phase 3: 리뷰 및 완료](#phase-3-리뷰-및-완료)
    - [8.8 develop (지휘자스킬)](#88-develop-지휘자스킬)
      - [Phase 0: 입력 확인](#phase-0-입력-확인-3)
      - [Phase 1: 개발 방식 선택](#phase-1-개발-방식-선택)
      - [Phase 2: AI Agent 개발 → Agent: agent-developer (`/oh-my-claudecode:ralph` 활용)](#phase-2-ai-agent-개발--agent-agent-developer-oh-my-claudecoderalph-활용)
      - [Phase 3: 빌드 오류 수정 (`/oh-my-claudecode:build-fix` 활용)](#phase-3-빌드-오류-수정-oh-my-claudecodebuild-fix-활용)
      - [Phase 4: QA/검증 (`/oh-my-claudecode:ultraqa` 활용)](#phase-4-qa검증-oh-my-claudecodeultraqa-활용)
      - [Phase 5: 완료 및 보고](#phase-5-완료-및-보고)
  ...
  - [12. 검증 계획](#12-검증-계획)
    - [단계별 검증](#단계별-검증)
    - [Phase 4 전체 검증 체크리스트](#phase-4-전체-검증-체크리스트)
