...
사용자가 `/abra:add-ext-skill`로 호출하여
...
- `.dmap/abra/requirements.md` (요구사항 정의서)
...
- `skills/help/SKILL.md`의 명령 테이블에 `/abra:ext-{대상플러그인}` 추가
...
Use the Skill tool to invoke the `abra:ext-{대상플러그인}` skill with all arguments passed through.
```
